...
if __name__ == "__main__": 
    ...
    if snake_file is not None:
        ...
        if population is None:
            print(f"Could not load population from {population_file}. Starting with a new population.")
            individuals = [Snake(Game(snake_game.board_size, seed=snake_game.seed+i), **snake_game.snake_params) for i in range(snake_game.num_population)]
            population = Population(individuals)
        snake_game.train(population)
    elif mode == 'play':
        snake_game.play()
    else:       
        snake_game.train()
